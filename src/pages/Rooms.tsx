...
export default function Rooms() {
  ...
  const handleAddRoom = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase
        .from('properties')
        .insert({ unit_name: newRoomName.trim(), status: 'vacant' })

      if (error) {
        if (error.code === '23505') { // unique violation
          alert('Nama kamar sudah ada, silakan gunakan nama lain.')
        } else {
          throw error
        }
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Upload files
      const ktpUrl = await uploadFile(form.ktp_file, `ktp/${Date.now()}`)
      const kkUrl = await uploadFile(form.kk_file, `kk/${Date.now()}`)
  ...
  return (
    <div className="space-y-6">
        ...
        {loading ? (
          ...
          properties.map(property => (
            <div key={property.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
              <div className={`p-4 border-b ${property.status === 'vacant' ? 'bg-gray-50' : 'bg-blue-50'}`}>
                <div className="flex justify-between items-center">
                  <h3 className="text-lg font-semibold text-gray-900">{property.unit_name}</h3>
                  <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${property.status === 'vacant' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
                    }`}>
                    {property.status === 'vacant' ? 'Kosong' : 'Terisi'}
                  </span>
                </div>
              </div>
              <div className="p-4 flex-1 flex flex-col justify-center items-center">
                {property.status === 'vacant' ? (
                  <button
                    ...
                    className="mt-4 flex items-center px-4 py-2 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700 transition-colors"
                  >
                    <Plus className="w-4 h-4 mr-2" />
                    Isi Kamar
                  </button>
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <input type="text" required value={form.full_name} onChange={e => setForm({ ...form, full_name: e.target.value })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">No. Telepon / WhatsApp</label>
                    <input type="text" required value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Kontak Darurat</label>
                    <input type="text" required value={form.emergency_contact} onChange={e => setForm({ ...form, emergency_contact: e.target.value })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Tanggal Masuk</label>
                    <input type="date" required value={form.start_date} onChange={e => setForm({ ...form, start_date: e.target.value })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md relative hover:bg-gray-50 cursor-pointer">
                        <input type="file" accept="image/*" required onChange={e => handleFileChange(e, 'ktp_file')} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                        <div className="space-y-1 text-center">
                          <Upload className="mx-auto h-8 w-8 text-gray-400" />
                          <div className="text-sm text-gray-600">
                            {form.ktp_file ? <span className="text-blue-600 font-medium truncate w-full block px-2">{form.ktp_file.name}</span> : <span>Pilih file KTP</span>}
                          </div>
                        </div>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md relative hover:bg-gray-50 cursor-pointer">
                        <input type="file" accept="image/*" required onChange={e => handleFileChange(e, 'kk_file')} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                        <div className="space-y-1 text-center">
                          <Upload className="mx-auto h-8 w-8 text-gray-400" />
                          <div className="text-sm text-gray-600">
                            {form.kk_file ? <span className="text-blue-600 font-medium truncate w-full block px-2">{form.kk_file.name}</span> : <span>Pilih file KK</span>}
                          </div>
                        </div>
                      </div>
                    </div>
      ...
      {isDetailModalOpen && selectedProperty && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen p-4">
            <div className="fixed inset-0 transition-opacity" aria-hidden="true" onClick={() => !isSubmitting && setIsDetailModalOpen(false)}>
              <div className="absolute inset-0 bg-gray-500 opacity-75"></div>
            </div>
            <span className="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div className="relative z-10 w-full max-w-lg bg-white rounded-lg text-left overflow-hidden shadow-xl">
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="flex justify-between items-center mb-5">
                  ...
                  <button onClick={() => !isSubmitting && setIsDetailModalOpen(false)} className="text-gray-400 hover:text-gray-500">
                    <X className="h-6 w-6" />
                  </button>
                </div>
                ...
                {isLoadingDetail ? (
                  ...
                  <div className="space-y-4">
                    <div className="bg-gray-50 p-4 rounded-md border border-gray-100">
                      ...
                      <p className="text-base text-gray-900 mb-3">{tenantDetail.full_name}</p>
                      ...
                      <p className="text-base text-gray-900 mb-3">{new Date(tenantDetail.start_date).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' })}</p>

                      <div className="flex gap-4 mt-4">
                        <a href={tenantDetail.ktp_url} target="_blank" rel="noreferrer" className="text-blue-600 text-sm font-medium hover:underline flex items-center">
                          Lihat KTP
                        </a>
                        <a href={tenantDetail.kk_url} target="_blank" rel="noreferrer" className="text-blue-600 text-sm font-medium hover:underline flex items-center">
                          Lihat KK
                        </a>
                      </div>
                    </div>
      ...
      {isAddRoomModalOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen p-4">
            <div className="fixed inset-0 transition-opacity" aria-hidden="true" onClick={() => !isSubmitting && setIsAddRoomModalOpen(false)}>
              <div className="absolute inset-0 bg-gray-500 opacity-75"></div>
            </div>
            <span className="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div className="relative z-10 w-full max-w-lg bg-white rounded-lg text-left overflow-hidden shadow-xl">
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="flex justify-between items-center mb-5">
                  ...
                  <button onClick={() => !isSubmitting && setIsAddRoomModalOpen(false)} className="text-gray-400 hover:text-gray-500">
                    <X className="h-6 w-6" />
                  </button>
                </div>
